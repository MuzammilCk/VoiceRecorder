...
class RealSongRecognitionService {
  ...
  private readonly ACCESS_KEY = process.env.NEXT_PUBLIC_ACRCLOUD_ACCESS_KEY || '';
  private readonly ACCESS_SECRET = process.env.NEXT_PUBLIC_ACRCLOUD_ACCESS_SECRET || '';
  ...
  constructor() {
    if (!this.ACCESS_KEY || !this.ACCESS_SECRET) {
      console.warn('ACRCloud API credentials not found. Please set NEXT_PUBLIC_ACRCLOUD_ACCESS_KEY and NEXT_PUBLIC_ACRCLOUD_ACCESS_SECRET environment variables.');
    }
  }
